...
async fn test_mutation_and_statement() {
    ...
    let result = async {
        ...
        return tx.update(CancellationToken::new(), stmt2).await;
    }.await;

    let result = tx.finish(CancellationToken::new(), result, None).await;
...
async fn test_rollback() {
    ...
    let result = async {
        ...
        tx.update(CancellationToken::new(), stmt2).await
    }
    .await;

    let _ = tx.finish(CancellationToken::new(), result, None).await;
    let session = create_session().await;
    let mut tx = read_only_transaction(session).await;
    let reader = tx
...
async fn assert_data(
    ...
    ",
        ...
        let result = tx.query(CancellationToken::new(), stmt).await?;
        all_rows(result).await
    }
    .await;

    // commit or rollback is required for rw transaction
    let rows = match tx.finish(CancellationToken::new(), result, None).await {
        Ok(s) => s.1,
