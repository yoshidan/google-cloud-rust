...
//!
//!     // Create topic.
//!     let topic = client.create_topic(ctx, "test-topic", None, None).await?;
//!
//!     // Publish message.
...
//!
//!     // Create subscription
//!     let subscription = client.create_subscription(ctx.clone(), "test-subscription", &topic, config, None).await?;
//!
//!     let ctx2 = ctx.clone();
